...
const fromCamelEntry = (e) => ({ ...(e.id ? { id: e.id } : {}), user_id: e.userId, date: e.date, calls: e.calls, emails: e.emails, appts: e.appts, notes: e.notes, from_deal: e.fromDeal ?? null, tagged_rep_id: e.taggedRepId || null, bdr_id: e.bdrId || null, company: e.company || null, ban: e.ban || null, fan: e.fan || null, contact: e.contact || null, phone: e.phone || null, phone_ext: e.phoneExt || null, email: e.email || null, carrier_rep: e.carrierRep || null, company_id: e.companyId || null });
const fromCamelDeal = (d) => ({ ...(d.id ? { id: d.id } : {}), owner_id: d.ownerId, company: d.company, contact: d.contact, contact_email: d.contactEmail || null, value: d.value, ...(d.commissionLines !== undefined ? { commission_lines: d.commissionLines || [] } : {}), stage: d.stage, close_date: d.closeDate || null, notes: d.notes, lost_reason: d.lostReason || null, next_action_date: d.nextActionDate || null, ...(d.apptAt !== undefined ? { appt_at: d.apptAt || null } : {}), ...(d.stageChangedAt !== undefined ? { stage_changed_at: d.stageChangedAt || null } : {}), appt_credited: d.apptCredited ?? false, tagged_rep_id: d.taggedRepId || null, company_id: d.companyId || null });
...
export async function updateEntry(id, patch) {
  const db = {};
  const map = { date: "date", calls: "calls", emails: "emails", appts: "appts", notes: "notes", company: "company", ban: "ban", fan: "fan", contact: "contact", phone: "phone", phoneExt: "phone_ext", email: "email", carrierRep: "carrier_rep", taggedRepId: "tagged_rep_id", bdrId: "bdr_id" };
  for (const k in map) {
    if (patch[k] !== undefined) {
      let v = patch[k];
      if (["calls", "emails", "appts"].includes(k)) v = Number(v) || 0;
      if (k === "taggedRepId" || k === "bdrId") v = v || null;
      db[map[k]] = v;
    }
  }
...
// Record a stage transition. Best-effort: history logging must never block the
// actual stage save, so callers can ignore a rejected promise here.
export async function logStageChange(dealId, fromStage, toStage) {
  const { data: { user } } = await supabase.auth.getUser();
  const { error } = await supabase.from("deal_stage_history").insert({
    deal_id: dealId, from_stage: fromStage || null, to_stage: toStage, changed_by: user?.id || null,
  });
  if (error) throw error;
}

export async function listStageHistory(dealId) {
  const { data, error } = await supabase.from("deal_stage_history")
    .select("*").eq("deal_id", dealId).order("changed_at", { ascending: true });
  if (error) throw error;
  return (data || []).map(toCamelStageEvent);
}
...
// ---- Contacts ----
const toCamelContact = (c) => c && ({ id: c.id, companyId: c.company_id, name: c.name, title: c.title || "", phone: c.phone || "", phoneExt: c.phone_ext || "", cellPhone: c.cell_phone || "", email: c.email || "", notes: c.notes || "", createdBy: c.created_by || "", createdAt: c.created_at });
...
export async function saveContact(companyId, contact) {
  ...
  const row = { company_id: companyId, name: contact.name, title: contact.title || "", phone: contact.phone || "", phone_ext: contact.phoneExt || "", cell_phone: contact.cellPhone || "", email: contact.email || "", notes: contact.notes || "" };
  if (contact.id) {
